		...
		<view class="l-daily-punch__days" v-if="currentMonth" :style="[daysStyle]">
			<view class="l-daily-punch__days-item" v-for="item in currentDays" :key="item.fullDate"
				...
				<view class="l-daily-punch__days-item--inner" :class="{
					...
					'is-checked-in': item.getIsCheckedIn() && item.type == 'current',
					'is-can-supplement': item.getCanSupplement() && item.type == 'current',
					'is-selected': item.isToday && selectDate == null || selectDate && selectDate.timestamp == item.timestamp,
...
<script lang="ts">
	...
	// import { isDarkMode } from '@/uni_modules/lime-style';
