...
class UserPassword {
	...
	//通过原密码修改密码
	async updatePassword(password, originPassword) {
		...
		const flag = await this.validPasswordIsCorrent(account_id, originPassword);
		if (!flag) {
			return {
				errMsg: "原密码不正确",
				errCode: 103
		...
		const res = await this.resetPassword(account_id, password);
		return res;
	}
	//重置密码
	async resetPassword(_id, password) {
		...
		const res = await db.collection("hm-user").doc(_id).update({
			password: ep
		});
		return res;
	}
	...
	async resetPasswordByEmailCode(email, emailCode, emailCodeTk, password) {
		try {
			...
			const res = await this.resetPasswordByEmail(email, password);
	...
	//检验密码是否正确
	async validPasswordIsCorrent(_id, password) {
		const db = uniCloud.databaseForJQL();
