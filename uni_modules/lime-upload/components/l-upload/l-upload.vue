<template>
	<view class="l-upload" ref="uploadRef" :style="[styles]">
		...
		<!-- #ifdef APP -->
		<template v-if="isApp && mediaType == 'all' && !isReadonly">
			<l-choose-file class="l-upload__item l-upload__item--add" :class="{'l-upload__item--disabled':isDisabled}"
				:style="[itemStyle]" aria-label="上传"
				:disabled="disabled"
				v-show="!multiple ? customFiles.length == 0 : max == 0 || customFiles.length != max" @success="onAddTap($event, true)">
		...
		<!-- #endif -->
		<view v-if="!(isApp && mediaType == 'all') && !isReadonly" class="l-upload__item l-upload__item--add" :class="{'l-upload__item--disabled':isDisabled}"
			:style="[itemStyle]" aria-label="上传"
...
<script lang="ts">
	...
	import { chooseFiles, normalizeChooseFiles } from './utils'
	import type { LimeUploadFile, ChooseFileOptions } from './type';
	export default defineComponent({
		...
		setup(props, { expose, emit }) {
			...
			const customFiles = ref<LimeUploadFile[]>(props.value ?? props.modelValue ?? props.defaultFiles ?? []);
			...
			let last = 0;
			const upload = (files : LimeUploadFile[]) => {
			...
			const onAddTap = (result, app) => {
				if (isReadonly.value || isDisabled.value) return
				if(app) {
					const res = result as ChooseFileSuccessCallbackResult
					const tempFilePaths = res.tempFiles
					const tempFiles = res.tempFiles
					const files = normalizeChooseFiles('all', tempFiles, tempFilePaths, props.sizeLimit, props.oversize)
			...
			const stop = watch(customFiles, (v : LimeUploadFile[]) => {
			...
			// #ifdef VUE2
			const stopValue = watch(() => props.value, (v : LimeUploadFile[]) => {
			...
			// #endif
			// #ifdef VUE3
			const stopValue = watch(() => props.modelValue, (v : LimeUploadFile[]) => {
				if (v.length != customFiles.value.length) {
					customFiles.value = v
				}
			})
