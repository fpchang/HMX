// @ts-nocheck
import type { LimeUploadFile, ChooseFileOptions, Oversize } from './type'
...
export function chooseAll(opts : ChooseFileOptions) {
	...
	// #ifndef MP-WEIXIN || UNI-APP-X && APP
	// #ifndef APP
	uni.chooseFile({
		count: opts.count,
		type: 'all',
	...
	opts.fail?.({
		errCode: 1101010,
		errMsg: 'uniapp app 不支持'
	})
	console.warn("【lime-upload】chooseFile 当前环境不支持")
	// #endif
	// #endif
...
export function normalizeChooseFiles(
	type: string, 
...
export function chooseFiles(opts: ChooseFileOptions):Promise<LimeUploadFile[]> {
